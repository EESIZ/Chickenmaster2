...
class Research:
    ...
    def calculate_progress_amount(self, stat_value: int, dice_value: int = None) -> int:
        """스탯에 따른 연구 진행량 계산 (README.md 공식 적용)
        
        공식: {1+[calldice/10]}*{관련 스테이터스}*(난이도 보정치)
        """
        if not self.can_start_research(stat_value):
            return 0
        
        # dice_value가 주어지지 않으면 DiceSystem에서 호출
        if dice_value is None:
            from game.DiceSystem import DiceSystem
            dice_value = DiceSystem.callDice()
        
        # README.md 공식 적용
        # 1. {1+[calldice/10]} 계산
        dice_factor = 1 + (dice_value / 10)
        
        # 2. {관련 스테이터스} - 실제 스탯값 사용
        stat_factor = stat_value
        
        # 3. (난이도 보정치) - 1/난이도
        difficulty_modifier = self.get_difficulty_modifier()
        
        # 최종 진행량 계산
        progress_amount = dice_factor * stat_factor * difficulty_modifier
        
        # 정수로 변환 (최소 1)
        return max(1, int(progress_amount))
